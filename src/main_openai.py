...
def process_pdf(pdf_path, prompt_file=None, pattern=None):
    prompt_file = _resolve_prompt(pdf_path, prompt_file=prompt_file, pattern=pattern)
    print(f"Using prompt: {prompt_file}")

    beams, folder, name, qa_report = run_structured_pipeline(pdf_path, prompt_file)
...
def main():
    ...
    for pdf_path in pdfs:
        process_pdf(pdf_path, prompt_file=args.prompt_file, pattern=args.pattern)
